...
def extract_scene_with_advanced_risk(
    nusc,
    scene_idx: int,
    ...
    if not sample['anns']:
        logger.warning("[risk_integration] No annotations in scene_idx=%s", scene_idx)
        return None
    ...
    for ann_token in sample['anns']:
        ...
        try:
            ...
            logger.exception("[risk_integration] box_velocity failed for ann_token=%s", ann_token)
            velocity = np.array([0.0, 0.0])
    ...
    if total_scene_risk > 0:
        for obj in objects_with_risk:
            obj['risk_attribution'] = round((obj['risk']['total_risk'] / total_scene_risk) * 100, 1)

    num_objects = len(objects_with_risk)
    avg_risk = total_scene_risk / num_objects if num_objects > 0 else 0

    scene_risk_level = "MINIMAL"
    if max_collision_risk >= 0.8 or min_ttc < 1:
        scene_risk_level = "CRITICAL"
    elif max_collision_risk >= 0.6 or min_ttc < 2:
        scene_risk_level = "HIGH"
    elif max_collision_risk >= 0.4 or min_ttc < 3.5:
        scene_risk_level = "MODERATE"
    elif max_collision_risk >= 0.2:
        scene_risk_level = "LOW"

    logger.info(
        "[risk_integration] scene_idx=%s objects=%s risk_level=%s max_collision=%.3f min_ttc=%s",
        scene_idx, num_objects, scene_risk_level, float(max_collision_risk),
        (round(min_ttc, 2) if min_ttc < 100 else None)
    )
...
def build_risk_aware_dataset(
    ...
    for scene_idx in range(min(num_scenes, len(nusc.scene))):
        scene_data = extract_scene_with_advanced_risk(nusc, scene_idx, ego_speed=ego_speed)
        ...
        if (scene_idx + 1) % 10 == 0:
            logger.info("[risk_integration] Built %d samples so far...", (scene_idx + 1))

    logger.info("[risk_integration] Finished dataset build. Total samples=%d", len(dataset))
    return dataset
...
def example_integration():
    ...
    """
    logger.info("=" * 70)
    logger.info("EXAMPLE: Risk-Aware Driving LLM Integration")
    logger.info("=" * 70)
    ...
    logger.info(detailed_prompt)

    logger.info("\n" + "=" * 70)
    logger.info("3. STRUCTURED PROMPT")
    logger.info("=" * 70)
    structured_prompt = generate_risk_aware_prompt(scene_data, 'structured')
    logger.info(structured_prompt)

    logger.info("\n" + "=" * 70)
    logger.info("4. TRAINING SAMPLE")
    logger.info("=" * 70)
    training_sample = generate_training_sample(scene_data)
    logger.info("Input length: %d", len(training_sample['input']))
    logger.info("Output:\n%s", training_sample['output'])
    logger.info("Metadata: %s", training_sample['metadata'])
